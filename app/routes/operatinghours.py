...
from fastapi import APIRouter, HTTPException, Depends, Path, Request, logger
from sqlalchemy.orm import Session
from app import crud_restaurants
...
# from app.schemas.RestaurantSchema import DayOfWeek,OperatingHoursResponse
...
@router.post("/manager/restaurants/{restaurant_id}/hours", response_model=RestaurantSchema.OperatingHoursResponse)
async def create_operating_hours(
    ...
    if user["role"] != "restaurant_manager":
        raise HTTPException(
            status_code=403,
            detail="Not authorized to create operating hours"
        )
    ...
    existing_hours = db.query(OperatingHours).filter(
    ...
    if existing_hours and existing_hours.day_of_week== operating_hours.day_of_week: 
        if not check_timeframes(operating_hours.opening_time, operating_hours.closing_time,
                         existing_hours.opening_time, existing_hours.closing_time):
            raise HTTPException(
                status_code=400,
    ...
    # Call the CRUD function to create the operating hours record
    db_operating_hours = OperatingHours(
        restaurant_id=operating_hours.restaurant_id,
        day_of_week=operating_hours.day_of_week,
        opening_time=operating_hours.opening_time,
        closing_time=operating_hours.closing_time
    )
    db.add(db_operating_hours)
    db.commit()
    db.refresh(db_operating_hours)
    if not db_operating_hours:
        raise HTTPException(status_code=400, detail="Failed to create operating hours") 
       
    return db_operating_hours

@router.get("/manager/restaurants/{restaurant_id}/hours", response_model=List[RestaurantSchema.OperatingHoursResponse])
async def get_operating_hours(
    ...
    if user["role"] != "restaurant_manager":
        raise HTTPException(
            status_code=403,
            detail="Not authorized to view operating hours"
        )
    ...
    operating_hours = db.query(OperatingHours).filter(
        OperatingHours.restaurant_id == restaurant_id
    ).all()
    if not operating_hours:
        raise HTTPException(status_code=404, detail="No operating hours found for this restaurant")
    
    return operating_hours


@router.put("/manager/restaurants/{restaurant_id}/hours/{hours_id}",
...
async def update_operating_hours(
    ...
    if user["role"] != "restaurant_manager":
        raise HTTPException(
            status_code=403,
            detail="Not authorized to update operating hours"
        )
    ...
    existing_record = db.query(OperatingHours).filter(
    ...
    conflicting_record = db.query(OperatingHours).filter(
        OperatingHours.restaurant_id == restaurant_id,
        OperatingHours.day_of_week == operating_hours.day_of_week,
        OperatingHours.hours_id != hours_id
    ).first()
    ...
    if conflicting_record:
        if not check_timeframes(
            ...
            conflicting_record.opening_time, conflicting_record.closing_time
        ):
            raise HTTPException(
                status_code=400,
                detail="Operating hours conflict: the specified time frame overlaps with an existing schedule for this day."
...
@router.delete("/manager/restaurants/{restaurant_id}/hours/{hours_id}", response_model=dict)
async def delete_operating_hours(
    ...
    if user["role"] != "restaurant_manager":
        raise HTTPException(
            status_code=403,
            detail="Not authorized to delete operating hours"
        )
